...
- **Diagrams:** always Mermaid (` ```mermaid `) or SVG — colorful, explanatory, with vivid clear colors harmonized with the mdBook theme. ASCII-art diagrams are **forbidden** in any documentation (ASCII sketches are fine in conversation); any ASCII diagram must be converted to a ` ```mermaid ` block. The book renders Mermaid client-side via `docs/skin/mermaid.min.js` + `mermaid-init.js` (registered in `docs/book.toml` `additional-js`), so a ` ```mermaid ` fence displays as a real diagram — never paste mermaid source as plain text. Verify every diagram renders (run `mdbook build docs`, then open the page) before committing.
- **Equations are LaTeX, not plain text:** any mathematical expression is written as an inline `$...$` or display `$$...$$` LaTeX block, rendered client-side by `docs/skin/katex*.js` (also in `additional-js`). Never write math as plain Unicode/ASCII (e.g. `‖a‖²`, `Σ`, `√`, `∫`). Plain `2×2`, prose arrows and code strings are fine; actual formulas are not.
- **Accessibility is mandatory:** every diagram and every rendered equation must be discoverable by a screen reader. Give every Mermaid diagram an `accTitle`/`accDescr` (rendered as an `aria-label`), and make sure equations read as spoken content (KaTeX annotates them automatically). Do not rely on colour alone to convey meaning; label axes, nodes and alternatives.
