...
| **Squared-Euclidean distance via the norm expansion $\|a\|^2 - 2a \cdot b + \|b\|^2$** | Expanding the square avoids a per-pair subtract loop, reuses a SIMD-friendly dot product over contiguous row memory, and is symmetric with a stable diagonal (spec `pairwise-distances`). | The literal $\sum (a_k - b_k)^2$ loop per pair — correct but slower and harder to vectorize. |
...
line between them: $\sqrt{\sum (a_k - b_k)^2}$. Its square is $\sum (a_k - b_k)^2$. Computing that
literally means subtracting each coordinate pair. But algebra gives us a shortcut by
expanding the square:

$$
\|a - b\|^2 = \sum (a_k - b_k)^2 = \sum (a_k^2 - 2 a_k b_k + b_k^2) = \|a\|^2 - 2(a \cdot b) + \|b\|^2
$$

where $\|a\|^2 = \sum a_k^2$ (the squared "length" of $a$) and $a \cdot b = \sum a_k b_k$ is the **dot
product** of $a$ and $b$. Why bother? Because we can precompute $\|a\|^2$ for *every* row once,
and $\|b\|^2$ for every row of the other set once, and then each cell only needs one dot
...
The cosine of the angle between $a$ and $b$ is $\frac{a \cdot b}{\|a\| \, \|b\|}$. If the vectors point the
same way the cosine is $1$; if they are perpendicular (orthogonal) it is $0$. Distance is
then defined as $1 - \text{cosine}$, so identical directions score $0$ and orthogonal directions
...
grid — only left/right and up/down, no diagonals: $\sum |a_k - b_k|$. It is the sum of the
...
multiplication $C = \alpha \cdot A \cdot B$. **LAPACK** builds on BLAS to provide the higher-level
...
$\|a\|^2 - 2a \cdot b + \|b\|^2$, with row norms precomputed and each cell needing one SIMD dot product;
Euclidean is its element-wise square root; Manhattan is $\sum |a_k - b_k|$; cosine is
$1 - \frac{a \cdot b}{\|a\| \, \|b\|}$, with a guard that maps zero-norm rows to distance $1$ (matching
scikit-learn).
...
*What we rejected:* The naive per-pair subtract loop $\sum (a_k - b_k)^2$ for squared Euclidean
...
2. `let distance = sk_squared_euclidean_distance_matrix(&left.view(), &right.view());` — we
   hand both sets in as views. Inside, the kernel computes $\|a_{\text{row}}\|^2$ for both rows
   ($0$ and $25$), $\|b_{\text{row}}\|^2$ for the single right row ($2$), then fills the $2 \times 1$ matrix
   with $\|a\|^2 + \|b\|^2 - 2(a \cdot b)$ per cell.
3. `assert!((distance[[0, 0]] - 2.0).abs() < 1e-12);` — the distance from `(0,0)` to `(1,1)`
   is $0 + 2 - 2(0 \cdot 1 + 0 \cdot 1) = 2$, matching the straight-line square $(1-0)^2 + (1-0)^2 = 2$.
...
```mermaid
flowchart LR
    accTitle: Modules of the sciencekit_math crate and their key functions
    accDescr: sciencekit_math groups kernels, layout, pairwise distances, sparse operations and a backend. Each module exposes free functions, and the backend module exposes the SKMathBackend trait implemented by faer, matrix multiply and ndarray-linalg.
    ...
    L --> L1["SKMemoryLayout: C, F, or Strided"]
    L --> L2["sk_force_contiguous returns a CowArray"]
    ...
    SO --> S2["sk_sparse_product uses sprs SMMP"]
...
- **Distances use math to win.** Squared-Euclidean is computed as $\|a\|^2 - 2a \cdot b + \|b\|^2$,
