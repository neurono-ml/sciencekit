...
- `sciencekit_common` (Phase 0.4): the automatic execution decision resolver (`sk_resolve_execution_plan` — turns `SKExecutionMode::Automatic` plus an `SKExecutionContext` into a concrete `SKExecutionPlan`), the mandatory builder foundation (`SKBuilder` trait + `SKBuilderState` defaulting to `Automatic`, `sk_validate_hyperparameter`, `SKReferenceEstimator` exemplar), observability (`SKOperationAttributes`/`sk_run_operation` tracing spans with an opt-in `observability-export` OpenTelemetry layer), and the `allocator-jemalloc`/`allocator-mimalloc` global-allocator feature flags. `sciencekit_math` exposes `sk_default_math_backend` (pure-Rust `faer` default, `blas-backend` opt-in) and per-backend `kind()` (`SKBackendKind`).
...
- Documentation: Phase 0 marked complete and the wave plan + six foundational technical decisions documented in the roadmap; `sciencekit_math` chapter restored; `nalgebra` small-matrix decision (ADR #37) recorded as deferred to W5.4 (PCA).
- Documentation theme (`docs/skin`, issue #60): body prose scaled to a 12pt-equivalent with a persistent A-/A/A+ control; top-bar icon buttons converted to borderless ghost controls and content buttons compacted; sidebar sections collapsed into a persisted single-open accordion with keyboard-accessible toggles (section links toggle in place when already viewed); sidebar resize restored to mdBook's variable-driven handle so content reflows instead of only shrinking; Mermaid diagrams standardized on fixed `--sk-*` fill+text pairs with opaque edge-label pills in both themes; diagram labels locked to Mermaid's assumed metrics with font-gated rendering so node text no longer clips.
- Documentation: `How to add an Algorithm` promoted to a primary-level entry in `SUMMARY.md` (no longer nested under Tutorials); URL unchanged.